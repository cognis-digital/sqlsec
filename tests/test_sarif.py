...
def test_sarif_driver_lists_every_rule():
    ...
    from sqlsec.deps import DEP_RULES
    from sqlsec.taint import TAINT_RULES

    # The catalog covers the regex rule set, the data-flow (taint) rules, and
    # the dependency-audit rules, so results from any engine resolve to a
    # descriptor.
    expected = ({r.rule_id for r in all_rules()}
                | set(TAINT_RULES) | set(DEP_RULES))
    assert ids == expected
